      ...
      for (Size_misfit in Size_crossloading) {
        ...
        if(Size_misfit == 0 && Type_misfit != "none"){
          ...
          if(save_as == 'local'){
            ...
            csv_path_lavaan <- file.path(save_dir, paste0("lavaan_", Exp_name_info, "_time_", Exp_time, ".csv"))
            write.csv(fitm_lavaan, file = csv_path_lavaan, row.names = FALSE) 
            csv_path_blavaan <- file.path(save_dir, paste0("blavaan_", Exp_name_info, "_time_", Exp_time, ".csv"))
            write.csv(fitm_blavaan, file = csv_path_blavaan, row.names = FALSE)
            csv_path_stan <- file.path(save_dir, paste0("stan_", Exp_name_info, "_time_", Exp_time, ".csv"))
