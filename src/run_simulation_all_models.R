...
foreach (core=1:8) %dopar% {
  ...
  # 1 and 2 only work on local, use 3
  # 1
  #current_dir <- dirname(rstudioapi::getActiveDocumentContext()$path) # RStudio
  # 2 Alternatively, for non-interactive scripts:
  ...
  # Number of cores for parallelisation (deprecated)
  #cores <- 1
        ...
        for (Size_misfit in Size_crossloading) {
          ...
          if(Size_misfit == 0 && Type_misfit != "none"){
            ...
            Exp_name_info <- paste(N_pers, N_timep, Type_misfit,
                                   Size_misfit, "core", core, "_version03_rand", sep = "_")
